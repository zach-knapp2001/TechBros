...
function find_adj_tiles(tile){
	...
	var up = [position[0]+2, position[1]];	
	var down = [position[0]-2, position[1]];
	...
	if(position[0] % 2 == 1) {
		...
		up_right = [position[0]+1, position[1]];
		down_right = [position[0]-1, position[1]];
		down_left = [position[0]-1, position[1]+1];
		up_left = [position[0]+1, position[1]+1];
	} else {
		// Test for adjacency on even tiles
		up_left = [position[0]+1, position[1]];
		down_left = [position[0]-1, position[1]];
	...
	for(i = 0; i < 6; i++) {
		...
		if (curr_adj_tile[0] < 0 || curr_adj_tile[1] < 0 || curr_adj_tile[0] > global.board_size[1] - 1 || curr_adj_tile[1] > global.board_size[0] - 1) {
